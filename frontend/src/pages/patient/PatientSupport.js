...
const FAQ = [
  {
    ...
    a: 'Open Find a doctor, search a specialty, then Book appointment and pick a date and time.'
  },
  {
    q: 'Can I cancel a visit?',
    a: 'Yes. Open Appointments and cancel any visit that is not already completed.'
  },
  {
    q: 'How do I update my health details?',
    a: 'Use Profile to edit contact info, allergies, conditions, and emergency contact.'
  }
];
...
const PatientSupport = () => {
  ...
  const onSubmit = async (e) => {
    ...
    setSubmitting(true);
    try {
      await patientService.submitSupport({
  ...
  return (
    <div className="mx-auto max-w-[960px] animate-fade-up">
      <div>
        ...
        <p className="mt-1 text-sm text-ink-500">Help for patients on Alive Health UG.</p>
      </div>
        ...
        <section className="rounded-2xl border border-ink-200/70 bg-white p-5 shadow-card sm:p-6">
          ...
          <form onSubmit={onSubmit} className="mt-5 space-y-4">
            ...
            <Dropdown
              ...
              options={CATEGORIES}
            />
            <TextInput label="Subject" name="subject" value={form.subject} onChange={onChange} required />
            <TextTextarea
              label="Message"
              name="message"
              value={form.message}
