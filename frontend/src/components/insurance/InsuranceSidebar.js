...
import { insuranceNav } from '../../data/insuranceDashboard';
...
const InsuranceSidebar = ({ open, onClose }) => {
    ...
    <>
      ...
      >
        <div className="mb-8 flex items-center justify-between px-2">
          <BrandLogo to="/insurance/home" subtitle="Insurance portal" nameClassName="text-sm" />
        ...
        <div className="scrollbar-hide flex-1 overflow-y-auto">
          ...
          <nav className="space-y-1">
            {insuranceNav.management.map((item) => {
          ...
          <nav className="space-y-1">
            {insuranceNav.configurations.map((item) => {
              const Icon = iconMap[item.icon];
              return (
        ...
        <div className="mt-4 rounded-2xl border border-ink-200 bg-ink-100/70 p-3">
          <div className="flex items-center gap-3">
            <div className="flex h-10 w-10 items-center justify-center rounded-full bg-brand-500 text-sm font-bold text-white">
              {(companyName?.[0] || 'I').toUpperCase()}
            </div>
            <div className="min-w-0 flex-1">
              <p className="truncate text-sm font-semibold text-ink-900">{companyName}</p>
              <p className="truncate text-xs text-ink-500">Insurer</p>
            </div>
            <button
              type="button"
