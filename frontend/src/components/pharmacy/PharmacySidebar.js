...
import { pharmacyNav } from '../../data/pharmacyDashboard';
...
const PharmacySidebar = ({ open, onClose }) => {
  const { user, requestLogout} = useAuth();
  const navigate = useNavigate();
  const pharmacyName = user?.pharmacyProfile?.pharmacyName || 'Pharmacy portal';
    ...
    <>
      ...
      >
        <div className="mb-8 flex items-center justify-between px-2">
          <BrandLogo to="/pharmacy/home" subtitle="Pharmacy portal" nameClassName="text-sm" />
        ...
        <div className="scrollbar-hide flex-1 overflow-y-auto">
          ...
          <nav className="space-y-1">
            {pharmacyNav.management.map((item) => {
          ...
          <nav className="space-y-1">
            {pharmacyNav.configurations.map((item) => {
              const Icon = iconMap[item.icon];
              return (
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="min-w-0 flex-1">
              <p className="truncate text-sm font-semibold text-ink-900">
                {user ? `${user.firstName} ${user.lastName}` : 'Pharmacist'}
              </p>
              <p className="truncate text-xs text-ink-500">{pharmacyName}</p>
            </div>
            <button
              type="button"
