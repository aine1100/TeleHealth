...
const PharmacyDetailPage = ({ role = 'patient' }) => {
  ...
  const medicines = useMemo(() => {
    if (Array.isArray(pharmacy?.medicines)) return pharmacy.medicines;
    return EMPTY_MEDICINES;
  }, [pharmacy]);
          ...
          <div className="flex items-center justify-between gap-3">
            <div>
              ...
              <p className="mt-0.5 text-xs text-ink-500">
                Paid orders appear here. Unpaid carts stay in Pharmacy orders until checkout.
              </p>
            </div>
            <Link
              to="/patient/orders"
